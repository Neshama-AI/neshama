"""
SiliconFlow Provider - 2026 全面升级版
国内聚合模型网关

文档: https://docs.siliconflow.cn/
认证: Bearer Token
特点: 国内可直连，免费 RPM=100, QPS=3
"""
...
class SiliconFlowProvider(BaseProvider):
    """SiliconFlow 提供商 - 2026 全面升级 (国内聚合)"""
    
    provider_name = "siliconflow"
    provider_display_name = "SiliconFlow (国内聚合)"
    ...
    # 模型分组
    MODEL_GROUPS = {
        "qwen": ["Qwen/Qwen2.5-7B-Instruct", "Qwen/Qwen2.5-72B-Instruct", "Qwen/Qwen2.5-Coder-32B-Instruct"],
    ...
    async def _make_request(
        ...
        async with session.post(
            ...
            if response.status != 200:
                ...
                raise Exception(f"SiliconFlow API error: {response.status} - {error_msg}")
    ...
    async def call(
        ...
        **kwargs
        ...
        if model is None:
            model = self.supported_models[0] if self.supported_models else "Qwen/Qwen2.5-7B-Instruct"
