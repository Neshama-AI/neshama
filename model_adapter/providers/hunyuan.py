"""
腾讯混元 (Hunyuan) Provider - 2026 全面升级版
腾讯混元大模型系列
...
class HunyuanProvider(BaseProvider):
    ...
    provider_display_name = "腾讯混元"
    
    # 2026 最新模型列表
    supported_models = [
        # 混元 2.0 系列
        "hunyuan-2.0-instruct",          # 混元 2.0 Instruct
        "hunyuan-2.0-think",             # 混元 2.0 Think (推理版)
        "hunyuan-2.0-pro",               # 混元 2.0 Pro
        # 混元 Lite
    ...
    # 模型分组
    MODEL_GROUPS = {
        "premium": ["hunyuan-2.0-pro", "hunyuan-2.0-think"],
        "standard": ["hunyuan-2.0-instruct"],
        "lite": ["hunyuan-lite"],
        "legacy": ["hunyuan", "hunyuan-standard", "hunyuan-pro"],
    }
    
    # 默认配置
    DEFAULT_BASE_URL = "https://api.hunyuan.tencent.com/v1"
    ...
    def _get_default_max_tokens(self, model: str) -> int:
        """获取模型默认最大 token 数"""
        defaults = {
            "hunyuan-2.0-instruct": 32000,
            "hunyuan-2.0-think": 32000,
            "hunyuan-2.0-pro": 32000,
            "hunyuan-lite": 8000,
            "hunyuan": 8000,
            "hunyuan-standard": 8000,
            "hunyuan-pro": 8000,
        }
        return defaults.get(model, 8000)
    ...
    async def _make_request(
        ...
        async with session.post(
            ...
            if response.status != 200:
                ...
                raise Exception(f"Hunyuan API error: {response.status} - {error_msg}")
    ...
    async def call(
        ...
        **kwargs
        ...
        if model is None:
            model = self.supported_models[0] if self.supported_models else "hunyuan-2.0-instruct"
        ...
        try:
            if stream:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        error_text = await response.text()
                        raise Exception(f"Hunyuan API error: {response.status} - {error_text}")
