"""
NVIDIA NIM Provider - 2026 全面升级版
NVIDIA NIM 推理微服务

文档: https://docs.nvidia.com/nim/
认证: Bearer Token (NGC API Key)
特点: 开发者计划免费原型
"""
...
class NVIDIAProvider(BaseProvider):
    ...
    # 模型分组
    MODEL_GROUPS = {
        "llama": ["meta/llama-3.3-70b-instruct", "meta/llama-3.1-8b-instruct", "meta/llama-3.1-70b-instruct"],
        "mistral": ["mistralai/mistral-nemo-12b-instruct", "mistralai/mixtral-8x7b-instruct-v0.1"],
        "google": ["google/gemma-2-9b-it", "google/recurrentgemma-2b"],
        "deepseek": ["deepseek-ai/deepseek-v3", "deepseek-ai/deepseek-r1"],
        "embedding": ["nvidia/nv-embed-qa-4"],
    }
    ...
    def _get_default_max_tokens(self, model: str) -> int:
        ...
        defaults = {
            ...
            "mistralai/mistral-nemo-12b-instruct": 4096,
    ...
    async def _make_request(
        ...
        async with session.post(
            ...
            if response.status != 200:
                ...
                raise Exception(f"NVIDIA NIM API error: {response.status} - {error_msg}")
    ...
    async def call(
        ...
        **kwargs
        ...
        if model is None:
            model = self.supported_models[0] if self.supported_models else "meta/llama-3.1-8b-instruct"
        ...
        try:
            if stream:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        error_text = await response.text()
                        raise Exception(f"NVIDIA NIM API error: {response.status} - {error_text}")
