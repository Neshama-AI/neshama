"""
OpenCode Provider - 2026 新增
OpenCode 编码助手适配

文档: https://opencode.ai/
支持: OpenCode Coding Plan API
"""
...
class OpenCodeProvider(BaseProvider):
    ...
    # 模型分组
    MODEL_GROUPS = {
        "openai": ["opencode-gpt-4o", "opencode-gpt-4.1"],
        "anthropic": ["opencode-claude-sonnet"],
    }
    ...
    def _get_default_max_tokens(self, model: str) -> int:
        """获取模型默认最大 token 数"""
        defaults = {
            "opencode-gpt-4o": 128000,
            "opencode-gpt-4.1": 128000,
            "opencode-claude-sonnet": 200000,
        }
    ...
    async def _make_request(
        ...
        async with session.post(
            ...
            if response.status != 200:
                ...
                raise Exception(f"OpenCode API error: {response.status} - {error_msg}")
    ...
    async def call(
        ...
        **kwargs
        ...
        if model is None:
            model = self.supported_models[0] if self.supported_models else "opencode-gpt-4o"
        ...
        try:
            if stream:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        error_text = await response.text()
                        raise Exception(f"OpenCode API error: {response.status} - {error_text}")
